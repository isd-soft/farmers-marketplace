...
const fetchChartData = async () => {
    try {
        const response = await axiosInstance.get(`/performance/category/revenue-sum?year=${props.year}`);
...
const setChartData = (performanceProjections) => {
    const categories = performanceProjections.map(projection => projection.category);
