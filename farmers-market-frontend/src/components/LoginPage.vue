      ...
      <TabPanels>
        <TabPanel value="0">
          ...
          <form>
            ...
            <ThemedButton @click="submitForm" class="login-btn" label="Login" />
          </form>
        </TabPanel>
        <TabPanel value="1">
          <!-- Register Form -->
      ...
      if (this.activeTab === '0') {
        ...
        if (!this.v$.email.$error && !this.v$.password.$error) {
          try {
            ...
            const responseRole = await axiosInstance.get(`/current-user/`);
            const userRole = responseRole.data.isAdmin
              ? 'ADMIN'
              : responseRole.data.isFarmer
                ? 'FARMER'
                : 'CUSTOMER';
