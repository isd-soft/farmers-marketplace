...
public class ProductServiceImpl implements ProductService {
    ...
    String notProductOwnerMessage = "You are not the owner of this product";

    @Override
    @Transactional
    public ProductDTO createProduct(CreateProductCommand createProductCommand) {
        JwtPrincipal principal = (JwtPrincipal) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        User creator = userRepository.findById(principal.getId()).orElseThrow(() -> new EntityNotFoundException(principal.getId(), User.class));
    ...
    @Override
    @Transactional
    public ProductDTO updateProduct(Long productId, UpdateProductCommand updateProductCommand) {
        if (isNotProductOwner(productId)) {
            throw new AccessDeniedException(notProductOwnerMessage);
        }
    ...
    public ProductDTO changeVisible(Long id, boolean visible) {
        if (isNotProductOwner(id) && isNotAppAdmin()) {
            throw new AccessDeniedException(notProductOwnerMessage);
        }
        Product product = productRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException(id, Product.class));
    ...
    public ProductPageDTO getProductPageById(Long productId) {
        ...
        var product = productRepository
                .findById(productId)
                .orElseThrow(() -> new EntityNotFoundException(productId, Product.class));
        if(!product.isVisible() && isNotProductOwner(product.getId())) {
            throw new AccessDeniedException("This product is unavailable");
        }
    ...
    private Image toImageEntity(String base64Image) {
    ...
    private boolean isNotProductOwner(Long productId) {
        JwtPrincipal principal = SecurityUtils.getPrincipal();
        assert principal != null;
    ...
    private boolean isNotAppAdmin() {
        ...
        return roles.stream()
                .noneMatch(role -> role.getRole() == ERole.ADMIN);
    }
}
